...
1. Download the file `SimplifiedMoveset.zip` from [Releases](https://github.com/SchuhBaum/SimplifiedMoveset/releases/tag/v2.7.3).
...
#### (Rain World v1.11)
v2.7.3:
- Partially reverted blacklisting `Void` and `Viy`. Now, only `Void` is
  blacklisted and only for the options `Wall Climb` and `Wall Jump`.

v2.7.2:
- Blacklisted slugcats `Void` and `Viy` from the mod `Last Wish`. The wall climb
  / jump option, etc. are incompatible.

v2.7.1:
- (swim) Fixed a bug where Rivulet still had finite breath underwater.
...
#### (Rain World v1.10)
v2.6.0:
- (swim) Fixed a bug where buoyancy was reset when using the MSC or Watcher DLC.

v2.5.9:
- Fixed two IL hooks that failed after the latest Rain Meadow update.

v2.5.8:
- (beam climb) Simplified fast beam climbing. When climbing up and pressing jump, you can cancel the slide up penalty by switching sides.

v2.5.7:
- (belly slide) Fixed a bug where you would stand up after a belly slide without using the option `Stand Up`.
