...
public static class PlayerMod
{
    ...
    private static void IL_Player_TerrainImpact(ILContext context) // Option_BellySlide || Option_Crawl || Option_Roll_1 || Option_Roll_2
    {
        ...
        if (cursor.TryGotoNext(instruction => instruction.MatchCall<Player>("get_input")))
        {
            ...
            cursor.EmitDelegate<Func<Player, IntVector2, float, bool>>((player, direction, speed) =>
            {
                ...
                if (player.animation == Player.AnimationIndex.CrawlTurn && Option_Crawl)
                {
    ...
    private static Player.ObjectGrabability Player_Grabability(On.Player.orig_Grabability orig, Player player, PhysicalObject physicalObject) // Option_Grab
    {
    ...
    private static void Player_Jump(On.Player.orig_Jump orig, Player player)
    {
        ...
        if (Option_WallJump && player.animation == Player.AnimationIndex.StandOnBeam && player.input[0].y > -1)
        {
            player.lowerBodyFramesOffGround = 0;
        }

        // prioritize retracting over jumping off beams;
        if (Option_TubeWorm && (player.IsClimbingOnBeam() || player.bodyMode == Player.BodyModeIndex.CorridorClimb) && player.IsTongueRetracting()) return;
        attachedFields.dontUseTubeWormCounter = 2;

        // don't do a rocket jump out of shortcuts
        if (player.shortcutDelay > 10 && player.animation == Player.AnimationIndex.BellySlide && Option_BellySlide) return;
        ...
        if (player.bodyMode != Player.BodyModeIndex.CorridorClimb && player.bodyMode != Player.BodyModeIndex.WallClimb)
        {
            ...
            if (Option_Roll_1 && player.animation == Player.AnimationIndex.Roll)
            {
            ...
            else
            {
                ...
                if (Option_BellySlide && player.animation == Player.AnimationIndex.BellySlide && player.rollCounter <= 8)
                {
                    player.bodyChunks[1].pos.y -= 10f;
                }

                // don't jump in the wrong direction when beam climbing
                if (Option_WallJump && player.animation == Player.AnimationIndex.ClimbOnBeam && player.input[0].x != 0)
                {
                ...
                if (Option_StandUp && player.superLaunchJump < 20 && (player.input[0].x != 0 || player.input[0].y > 0 || player.input[0].y > -1 && player.bodyChunks[0].contactPoint.y == -1))
                {
    ...
    private static void Player_ThrowObject(On.Player.orig_ThrowObject orig, Player player, int grasp, bool eu) // Option_BellySlide || Option_SpearThrow
    {
        ...
        if (Option_SpearThrow && player.grasps[grasp]?.grabbed is Weapon && player.animation != Player.AnimationIndex.BellySlide && (player.animation != Player.AnimationIndex.Flip || player.input[0].y >= 0 || player.input[0].x != 0))
        {
    ...
    private static void Player_UpdateAnimation(On.Player.orig_UpdateAnimation orig, Player player)
    {
        ...
        if (Option_Crawl && player.animation == Player.AnimationIndex.CorridorTurn && player.corridorTurnCounter < 30)
        {
            player.corridorTurnCounter = 30;
        }

        if (Option_CrouchJump && player.bodyMode == Player.BodyModeIndex.Crawl && player.superLaunchJump > 0 && player.superLaunchJump < 10)
        {
            player.superLaunchJump = 10;
        }

        // belly slide // backflip always possible // do a longer version by default
        if (Option_BellySlide && player.animation == Player.AnimationIndex.BellySlide)
        {
        ...
        else if (Option_Swim && player.animation == Player.AnimationIndex.DeepSwim)
        {
            // don't update twice;
        ...
        // ledge grab 
        else if (Option_WallJump && player.animation == Player.AnimationIndex.LedgeGrab && (player.canWallJump == 0 || Math.Sign(player.canWallJump) == -Math.Sign(player.flipDirection)))
        {
        ...
        {
            orig(player);
            if (player.animation == Player.AnimationIndex.None && Option_StandUp && (player.IsTileSolid(0, 0, -1) || player.IsTileSolid(1, 0, -1))) // stand up after rocket jump
            {
        ...
        {
            orig(player);
            if (player.animation == Player.AnimationIndex.None && Option_StandUp && (player.IsTileSolid(0, 0, -1) || player.IsTileSolid(1, 0, -1))) // stand up after belly slides // don't try to stand up when sliding down walls
            {
                AlignPosYOnSlopes(player);
                player.standing = true;
                player.animation = Player.AnimationIndex.StandUp;
    ...
    private static void Player_UpdateBodyMode(On.Player.orig_UpdateBodyMode orig, Player player)
    {
        ...
        if (Option_SlideTurn && player.initSlideCounter > 0 && player.initSlideCounter < 10)
        {
    ...
    private static void Player_WallJump(On.Player.orig_WallJump orig, Player player, int direction)
    {
        ...
        if (Option_WallClimb && !player.GetAttachedFields().initializeHands)
        {
    ...
    private static Vector2 Tongue_AutoAim(On.Player.Tongue.orig_AutoAim orig, Player.Tongue tongue, Vector2 direction) // Option_TubeWorm
    {
    ...
    private static void Tongue_Shoot(On.Player.Tongue.orig_Shoot orig, Player.Tongue tongue, Vector2 direction) // Option_TubeWorm
    {
